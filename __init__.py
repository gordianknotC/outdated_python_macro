...
class MacroExecutionError(Exception):
    def __init__(self, msg, pragname, node, kargs):
        ...
        msg0 = '\n  File "{}", line {}, in {}\n'.format(kargs.get('caller_filename', 'Unknown'), node.lineno, funcname)
        pragname = 'some error occured when executing {} fn\n'.format(pragname)
        msg = msg0 + pragname + str(msg)
...
def fix_locations(node, old_node):
    ...
    *old_node* (we cannot preserve original location information for code from
    ...
    def _fix(node, lineno, col_offset):
        # todo: maybe bugs contains in this if test?
        if 'lineno' in node._attributes:
            if source_map is not None:
                print('_fix add sourcemap', lineno, node.lineno, to_source(node).split('\n',1)[0])
                source_map.append(origLine=lineno, generatedLine=node.lineno)
            node.lineno     = lineno
    ...
    if is_n_list and is_o_list:
        ...
        if old_node and node:
            return fix_locations(node, [old_node], source_map=source_map)
...
class BaseExpander(ast.NodeTransformer):
    ...
    def _handle_ast_decorator_macro(self,node,macro_def, decor_call=False):
        ...
        try:
            nbody_gen_from_macro = execMacro(
                 ...
                 kargs=caller_kargs,          caller_filename = self.filename )

        except Exception as err:
            raise MacroExecutionError(err, macro_def.func_name, caller,
                                      dict(filename=self.filename,context_node=None))
    ...
    def _handle_astExpr(self, caller, macro_def):
        ...
        # ---------------------------------------------------------------------------
        try:
            print('_handle_astExpr', caller, to_source(caller), macro_func, macro_def.__dict__['func_name'])
            nbody_gen_from_macro = execMacro(
                 macro_func,     caller_node, caller_args,
...
class Expander(BaseExpander):
    ...
    def transform_completed(self, fix):
        fix(self.tree)
...
class MacroImporter(object):
    ...
    def load_module(self, name):
        ...
        if info[2] == imp.PY_SOURCE:
            ...
            print('imp.load_module:', name, fd, fn, info)
        ...
        try:
            ...
            try:
                ...
                #print('visit tree:')
                transformer = Expander(module, filename=filename, tree=tree)
            ...
            transformer.transform_completed(ast.fix_missing_locations)
            ...
            if name in redirector:
                ...
                if hook_mode == 'rapydscript':
                    source = to_source(transformed, filename=target, record_sourcemap=True)
            ...
            if hook_mode != 'rapydscript': return module
            else:
                module.__ast__ = transformed
        except Exception as err:
            #print('-----------------------------------------')
            # print('---------------------------------------')
            # print('---------------------------------------')
...
def install_hook(mode='macro', transformer = None, install_writer=None):
    """Install the import hook to intercept module import"""
    global hook_mode, to_source, Expander
    hook_mode = mode
    if mode == 'rapydscript':
        def _to_source(*args, **kargs):
            return codegen.to_source(*args, strip_annotation=True, install_writer=install_writer, **kargs)
...
def is_outdated(from_file, to_file, outdated=None, duration=100):
    ...
    if outdated is None:
        ...
        #----------------------------------------------------
        div = stat1 - stat2
        if abs(div) < duration:  outdated = False
        else:                    outdated = True
    print('file1:', os.path.basename(from_file), 'file2:', os.path.basename(to_file), 'is oudate:', outdated)
    return outdated
...
def redirect(to_file=None, export=None):
    def real(from_file, module_name, to_file = to_file, export=export):
        ...
        if to_file == 'pyj':
            to_file = from_file.replace('.py', '.pyj')
...
def set_debug_mode(dbg=False):
    global is_outdated
    def _is_outdated(from_file, to_file, outdated=None, duration=100):
        return True
    if dbg:
